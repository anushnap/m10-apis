...
Director <- function(movie.info) {
  statement <- paste("The director of", movie.info$Title, "is", movie.info$Director)
  return (statement)
}
...
movie.list <- c("Mean Girls", "The Curious Case of Benjamin Button", "The Girls Who Leapt Through Time")

# Remove spaces from each movie title
no.spaces <- gsub(" ", "+", movie.list)
# Prepare this list to be passed in as parameters for the API
search <- paste0("t=", no.spaces, "&r=json")

# Create API URL request and assign it to the variable api.request
api.request <- paste0(base, search)
